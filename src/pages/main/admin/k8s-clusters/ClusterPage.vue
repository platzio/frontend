    ...
    <div>
        ...
        <div class="my-4 card">
            ...
            <div class="card-body" v-if="cluster.env_id">
                <div>
                    This cluster is currently assigned to the following env:
                </div>
                <div class="my-3 h5">
                    <PlatzEnv :id="cluster.env_id" :showInfo="false" />
                </div>
                <div class="my-2">
                    You can change the env, which would immediately move all of
                    its deployments to the new env.
                </div>
                <div class="my-2">
                    If you unassign the env without first shutting down its
                    deployments, all deployments would keep on running without
                    you or anyone else being able to control them until a new
                    cluster is assigned.
                </div>
                <div class="mt-3">
                    <button
            ...
            <div class="card-body" v-else>
                <div>
                    This cluster is not assigned to any env. This means that
                    nobody can run deployments on it.
                </div>
        ...
        <div class="my-4 card">
            ...
            <div class="card-body">
                <div class="mb-2">
                    These settings control the injected ingress section when
                    installing deployments.
                </div>
                <div class="mb-2">
                    When a deployment has the ingress feature enabled in its
                    features.yaml, Platz creates a domain name under the ingress
                    domain. If an ingress class is set, it's also injected to
                    the deployment chart values when installing.
                </div>
                <div class="mb-2">
                    Ingress TLS secret name turns on TLS for the ingress. Note
                    that Platz doesn't check if the secret exists: This allows
                    for using both a wildcard certificate replicated to all
                    namespaces by services such as Reflector, or having a
                    certificate created in the chart itself.
                </div>
                <div class="my-3">
                    ...
                    <span class="text-body-secondary fst-italic" v-else
                        >(not set)</span
                    >
                </div>
                <div class="my-3">
                ...
                <div class="my-3">
                    ...
                    <span
                        class="fw-bold"
                        v-if="cluster.ingress_tls_secret_name"
                    >
                ...
                <button
                    class="btn btn-outline-primary"
                    @click="
                        editIngressSettings && editIngressSettings.open(cluster)
                    "
                >
        ...
        <div class="my-4 card">
            ...
            <div class="card-body" v-if="cluster.ignore">
                ...
                <div class="my-2">
                    This means that Platz doesn't track its status and any
                    deployments running in it are completely ignored.
                </div>
                <div class="my-2">
                    You can un-ignore the cluster, which would cause Platz to
                    resume monitoring the cluster.
                </div>
                <div class="mt-3">
                    <button
                        class="btn btn-outline-primary"
            ...
            <div class="card-body" v-else>
                <div class="mb-2">
                    This cluster is
                    <strong class="text-success">not ignored</strong>. Platz
                    monitors this cluster and all of its deployments.
                </div>
                <div class="my-2">
                    If you ignore this cluster, Platz will stop monitoring it.
                    This is useful when clusters are taken down, allowing Platz
                    to keep a full history of all actions that happened on old
        ...
        <div class="my-4 card">
            ...
            <div class="card-body">
                <div class="mb-3">
                    Configure a Grafana URL and a Loki data-source name to show
                    an "Open Logs" action for deployments running in this
                    cluster.
                </div>
                <div class="my-3">
                    <span>URL: </span>
                ...
                <div class="my-3">
                    ...
                    <span
                        class="fw-bold"
                        v-if="cluster.grafana_datasource_name"
                        >{{ cluster.grafana_datasource_name }}</span
                    >
                    <span class="text-body-secondary fst-italic" v-else
                        >(not set)</span
                    >
