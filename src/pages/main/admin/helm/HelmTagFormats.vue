<template>
  <div v-if="tagFormats">
    ...
    <div class="my-4">
      <div class="my-3">
        The formats below are how Helm chart tags are parsed into version,
        revision, Git branch and commit.
      </div>

      <div class="my-3">
        You can add new tag formats, which triggers a parsing of all existing
        Helm charts. Tag format patterns are Perl-style regular expressions that
        may contain the following named groups:
        <strong>version</strong>, <strong>revision</strong>,
        <strong>branch</strong> and <strong>commit</strong>. If a named group is
        missing from the pattern, it will be stored as an empty string in case
        the rest of the pattern successfully matches the image tag.
      </div>

      <div class="my-3">
        When trying to parse a Helm tag, formats are tested by their creation
        time from newest to oldest. This means that if you add a new tag format
        it may potentially supersede existing tag formats.
      </div>

      <div class="my-3">
        If a tag format doesn't match any Helm chart you can safely delete it.
        Tag formats currently matching any Helm chart may not be deleted.
      </div>
    </div>
        ...
        <PlatzItemWithActions>
          ...
          <template #actions>
            <li>
              <a
                class="dropdown-item"
                @click="deleteTagFormat && deleteTagFormat.open(item)"
              >
