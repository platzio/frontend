          ...
          <div class="mb-1 d-flex flex-row align-items-start">
            <div class="flex-fill">
              <template v-if="numAdmins === 0">
                ...
                <div>
                  Add yourself or another user as an env admin to control env users, deployment
                  permissions and env secrets.
                </div>
              </template>
              <template v-else>
                <div class="mb-1">As a site admin, you can manage users for all envs.</div>
                <div>
                  If you don't want to see an env in the env switcher, you can remove yourself from
                  the env and always add yourself later.
                </div>
              </template>
            </div>
            <router-link
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="text-secondary">
            When enabled, new users signing into Platz will be added to this env automatically. New
            users are added with the User role and be upgraded to Admins manually.
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="card-body">
          <div class="mb-4 d-flex flex-row align-items-start">
            <div class="text-secondary flex-fill">
              These settings control the nodes on which env deployments can run on. Note that once
              changed, all deployments of this env are automatically redeployed.
            </div>
            <button
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const nodeSelector = computed(() => yaml.dump(env.value.node_selector).trimEnd());
